...
| P8 | Навигация по документу (outline) | Outline заголовков в сайдбаре ниже дерева файлов; клик — прокрутка к заголовку в WYSIWYG (на рассмотрение) |
...
1. **Расположение приложения.** Репозиторий уже называется `vasyavig`, а §3 показывает корень проекта как `vasyavig/`. Создавать приложение **в корне репозитория** (src/, src-tauri/ рядом с _docs_) или во вложенной папке `app/` (как в предыдущем проекте transcript-optimizer)?
...
2. **LESSONS_LEARNED.md.** Файла в корне нет — его содержимое уже вшито в §15. Создать отдельный `LESSONS_LEARNED.md` по старой традиции (и оставить §15 как ссылку), или §15 в идее достаточно?
   - *Рекомендация ИИ:* создать отдельный файл — REFRESH_CONTEXT.md требует его чтения каждой сессией.
