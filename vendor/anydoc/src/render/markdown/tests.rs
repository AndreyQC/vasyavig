...
#[test]
fn image_destination_underscores_not_escaped() {
    // vasyavig phase 5: `\`-escaping an underscore in an image destination is
    // a legacy artifact that downstream consumers choke on — `format_url`
    // must leave `_` bare in both bare and angle-bracketed destinations.
    let img = |alt: &str, url: &str| {
        Block::Paragraph(vec![Inline::Image { alt: alt.into(), source: ImageSource::External(url.into()) }])
    };
    let md = doc(vec![
        img("рис", "assets/my_pic.png"),
        img("", "V2_ТЗ Total (1)_assets/img-0.png"),
    ]);
    assert_eq!(
        md,
        "![рис](assets/my_pic.png)\n\n![](<V2_ТЗ Total (1)_assets/img-0.png>)\n"
    );
}

#[test]
fn asset_destination_underscores_not_escaped() {
    use super::document_to_markdown_with_assets;
    ...
    asset_links.insert(AssetId(0), "V2_ТЗ Total (1)_assets/img-0.png".to_string());
    let md = document_to_markdown_with_assets(&doc, &asset_links);
    assert_eq!(md, "![рис](<V2_ТЗ Total (1)_assets/img-0.png>)\n");
}
