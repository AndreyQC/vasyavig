...
use crate::model::{AssetId, Block, Document, Inline, List, MarkerKind, Note, TableKind, inlines_are_empty};
...
pub(crate) struct Ctx {
    nums: NoteNumbers,
    anchors: AnchorMap,
    /// Embedded-image asset id → markdown link path (empty map = alt text only).
    asset_links: HashMap<AssetId, String>,
}
...
pub fn document_to_markdown(doc: &Document) -> String {
    let rc = Ctx {
        ...
        asset_links: HashMap::new(),
    };
    render_document(doc, &rc)
}

/// Renders `doc` with embedded images linked to the caller-provided relative paths.
pub(crate) fn document_to_markdown_with_assets(
    doc: &Document,
    asset_links: &HashMap<AssetId, String>,
) -> String {
    let rc = Ctx {
        nums: number_notes(doc),
        anchors: resolve_anchors(doc),
        asset_links: asset_links.clone(),
    };
    render_document(doc, &rc)
}

fn render_document(doc: &Document, rc: &Ctx) -> String {
