...
/// Convert a document to Markdown, writing embedded assets (images, object
/// payloads) into `assets_dir` and linking them from the Markdown as
/// `![alt](<assets_dir name>/<stem>-<id>.<ext>)`.
///
/// Link paths are relative to the directory that will hold the Markdown file,
/// which is the parent of `assets_dir`.
pub fn to_markdown_with_assets(path: impl AsRef<Path>, assets_dir: &Path) -> Result<String, ConvertError> {
    let path = path.as_ref();
    let bytes = std::fs::read(path)?;
    let Some(format) = Format::from_bytes(&bytes).or_else(|| Format::from_path(path)) else {
        return Err(ConvertError::Unsupported(format!(
    ...
    for asset in &doc.assets {
        ...
        let ext = if kind == "image" {
            subtype.chars().filter(|c| c.is_ascii_alphanumeric()).collect::<String>()
        } else {
            "bin".to_string()
        };
        let file = format!("{stem}-{}.{ext}", asset.id.0);
        std::fs::write(assets_dir.join(&file), &asset.bytes)?;
        links.insert(asset.id, file);
    }
    ...
    let links = links
        .into_iter()
        .map(|(id, file)| (id, format!("{prefix}/{file}")))
        .collect();
    Ok(render::markdown::document_to_markdown_with_assets(&doc, &links))
}
